 ...
 */
function PushStatePanel({ state, onInstallHelp }: { state: PushState; onInstallHelp: () => void }) {
  ...
  const copy: Record<string, { title: string; body: string; action?: string }> = {
    ...
    'ios-needs-install': {
      ...
      body: 'On iPhone and iPad, notifications only reach an installed app, never a Safari tab. It takes three taps.',
      action: 'Show me how',
    },
    denied: {
      title: 'Notifications are blocked',
      body: 'This site was refused permission, and only your browser settings can give it back — Meridian cannot ask again. On iOS: Settings → Notifications → Meridian.',
    },
    ...
    <div className="neu-pressed mb-3 rounded-card px-4 py-3.5">
      ...
      {entry.action && (
        <button
          ...
          className="mt-2.5 min-h-[38px] rounded-full px-4 text-[12.5px] font-semibold text-ink focus:outline-none focus-visible:ring-1 focus-visible:ring-accent"
...
export default function MeridianSettings() {
  ...
  // The iOS explanation surfaces itself whenever it is the actual blocker, which
  // is the "re-surfaceable" half of the dismissible banner on the launcher.
  useEffect(() => {
    if (pushState === 'ios-needs-install') setShowInstallHelp(true)
  }, [pushState])
  ...
  return (
    <div className="mx-auto min-h-full max-w-lg px-5 pb-20 pt-safe-top">
      ...
      >
        {!vapidConfigured() && (
          <div className="neu-pressed mb-3 rounded-card px-4 py-3.5">
            <h3 className="text-[13.5px] font-semibold text-expense">Not configured yet</h3>
            <p className="mt-1 text-[12px] leading-relaxed text-muted">
              <code className="text-primary">VITE_VAPID_PUBLIC_KEY</code> is missing from this build. Run{' '}
              <code className="text-primary">npm run vapid</code>, then add it to <code className="text-primary">.env.local</code> and to
              your deployment’s environment variables.
            </p>
          </div>
        )}

        <PushStatePanel state={pushState} onInstallHelp={() => setShowInstallHelp(true)} />

        {showInstallHelp && installGate.needed && (
          <div className="mb-3">
            <IosInstallBanner variant="inline" onDismiss={() => setShowInstallHelp(false)} />
          </div>
        )}
